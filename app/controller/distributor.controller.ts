    ...
    for (const addData of [req.body.CityId].flat()) {
      ...
      if (findCity) {
        await disCityRepo.upsert(
          ...
          {
            conflictPaths: [
              nameOf<DistributorCityEntity>("DistributorCityId"),
              nameOf<DistributorCityEntity>("DistributorId"),
              nameOf<DistributorCityEntity>("DistributorCityName"),
            ],
          }
        );
      }
...
export const updateDistCity = async (req: CustomRequest, res: Response) => {
  if (!req.query.DistributorCityId) {
    return new ErrorResponse(
      ...
      "Please provide DistributorCityId!!!"
    ).send(res);
  }
  const database = await connectToDatabase();
  const distributorRepo = database.getRepository(DistributorCityEntity);
  ...
  if (findCity) {
    await distributorRepo.update(
      req.query.DistributorCityId as any,
      { ...req.body }
    );
    return new SuccessResponse(
      StatusCodes.OK,
